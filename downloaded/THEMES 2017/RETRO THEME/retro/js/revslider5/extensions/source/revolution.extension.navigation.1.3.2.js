...
jQuery.extend(true,_R, {
	...
	manageNavigation : function(opt) {
		...
		if (ckNO(opt.navigation.bullets)) {
			...
			// START: retrowp
			if ( 'retro' !== opt.navigation.bullets.style ) {
			    setNavElPositions(opt.c.find('.tp-bullets'),opt.navigation.bullets,opt);
			}
			// END: retrowp
		}
	...
	// MANAGE THE NAVIGATION
	 createNavigation : function(container,opt) {
		...
		container.bind('revolution.slide.onafterswap revolution.nextslide.waiting',function() {		
			...
			si = _container.find(".next-revslide").length==0 ? _container.find(".active-revslide").data("index") : _container.find(".next-revslide").data("index");
			_container.find('.tp-bullet').each(function() {
			// END: retrowp
				var _t = jQuery(this);		
				if (_t.data('liref')===si) 
		...
		// START: retrowp
		if( 'retro' === _b.style ) {
		    container.bind('revolution.slide.onafterswap',function() {
			paginTimeLine.restart( container, opt.delay );
		    });
		    container.bind('revolution.nextslide.waiting',function() {
			paginTimeLine.reset( container );
		    });
		}
		// END: retrowp
		...
		cp.on("mouseenter mousemove",function() {
			...
			if (!cp.hasClass("tp-mouseover")) {
				...
				// START: retrowp
				if (a && _a.hide_onleave && 'retro' !== _a.style) showHideNavElements(cp.find('.tparrows'),_a,"show");
				if (b && _b.hide_onleave && 'retro' !== _b.style) showHideNavElements(cp.find('.tp-bullets'),_b,"show");
				// END: retrowp	
		...
		// START: retrowp
		if (a && _a.hide_onleave && 'retro' !== _a.style) showHideNavElements(cp.find('.tparrows'),_a,"hide",0);
		if (b && _b.hide_onleave && 'retro' !== _b.style) showHideNavElements(cp.find('.tp-bullets'),_b,"hide",0);
		// END: retrowp
...
var callAllDelayedCalls = function(container,opt) {
	...
	// START: retrowp
	if (ckNOLO(opt.navigation.arrows) && 'retro' !== opt.navigation.arrows.style)
	// END: retrowp
	...
	// START: retrowp
	if (ckNOLO(opt.navigation.bullets) && 'retro' !== opt.navigation.bullets.style)
	// END: retrowp
...
var initArrows = function(container,o,opt) {
	...
	var _container = container;
	if ( 'retro' === o.style ) {
	    if ( container.parent().find( '.tp-bullets' ).length === 0 ) {
		container.parent().append( '<div class="tp-bullets ' + o.style + ' ' + 'horizontal' + '"><span></span></div>' );
	    }
	    container = container.parent().find( '.tp-bullets' ).find( 'span' ).eq( 0 );
	}
	// END: retrowp	
	...
	if (o.rtl) {
		...
		// START: retrowp
		la.click(function() { opt.sc_indicator="arrow"; opt.sc_indicator_dir = 0;_container.revnext();});
		ra.click(function() { opt.sc_indicator="arrow"; opt.sc_indicator_dir = 1;_container.revprev();});
		// END: retrowp
	} else {
		// CLICK HANDLINGS ON LEFT AND RIGHT ARROWS
		// ORIGINAL: retrowp
		...
		// START: retrowp
		ra.click(function() { opt.sc_indicator="arrow"; opt.sc_indicator_dir = 0;_container.revnext();});
		la.click(function() { opt.sc_indicator="arrow"; opt.sc_indicator_dir = 1;_container.revprev();});
		// END: retrowp
	}
	// SHORTCUTS
...
var addBullet = function(container,o,li,opt) {
	// START: retrowp
	var li_length = opt.allli.length;
    var _container = container;
	if ( 'retro' === o.style ) {
	...
	if (container.find('.tp-bullets').length===0) {
		...
		// START: retrowp
		if ( 'retro' === o.style ) {
		    container.append('<div class="tp-bullets '+o.style+' '+o.direction+'"><span></span></div>');
		} else {
		    container.append('<div class="tp-bullets '+o.style+' '+o.direction+'"></div>');
		}
		// END: retrowp
	...
	var _bw = bw;
	if ( 'retro' === o.style ) {
	    bw = container.find('.tp-bullets span');
	}
	// END: retrowp
	jQuery.each(opt.thumbs[li.index()].params,function(i,obj) { inst = inst.replace(obj.from,obj.to);})
    // START: retrowp
	if ( 'retro' === o.style ) {
        var _index = '<div class="retro-basic">'+(li.index()+1)+'</div><div class="retro-more">'+((li.index()+1)+'/'+li_length)+'</div>';
		inst = inst.replace( /\{\{index\}\}/g, _index );
	    if ( 0 < bw.find( '.tp-bullet' ).length ) {
		bw.append( '<div class="separator"><div style="width: 0px; overflow: hidden;"></div></div>' );
	    }
	}
	// END: retrowp
	...
	b.click(function() {
        ...
        // START: retrowp
		_container.revcallslidewithid(linkto);
		// END: retrowp
		container.find('.tp-bullet').removeClass("selected");
		jQuery(this).addClass("selected");
        // START: retrowp
		paginTimeLine.restart( container, opt.delay - 100 );
		// END: retrowp
	...
	_bw.addClass("nav-pos-hor-"+o.h_align);
	// END: retrowp
	...
	if ( 'retro' !== o.style ) {
        ...
        var more_max_visible = function () {
            if ( o.max_visible < li_length) {
                bw.addClass( 'has-more-max-visible' );
            } else {
                bw.removeClass( 'has-more-max-visible' );
                if ( container.find( '.tp-bullets' ).width() <= bw.width() ) {
                    bw.addClass( 'has-more-max-visible' );
                }
            }
        }
