...
} else if ( 'retro-sm' === $style || 'retro-brd' === $style || 'retro-txt' === $style ) {
	$uid = uniqid();
	
	$button_css = array();
	$button_css_hover = array();
	if ( 'retro-txt' === $style ) {
		$button_css[] = 'color: ' . $retro_color;
		$button_css_hover[] = 'color: ' . $retro_color_hover;
	} else {
		$button_css[] = 'background-color: ' . $retro_color;
		$button_css_hover[] = 'background-color: ' . $retro_color_hover;
		$button_css[] = 'color: ' . $retro_color_text;
		$button_css_hover[] = 'color: ' . $retro_color_text_hover;
	}
	echo '<style type="text/css">.vc_btn3-style-' . $style . '.vc_btn-retro-btn-' . $uid . ':hover{' . implode( ';',
			$button_css_hover ) . ';' . '}</style>';
	echo '<style type="text/css">.vc_btn3-style-' . $style . '.vc_btn-retro-btn-' . $uid . '{' . implode( ';',
			$button_css ) . ';' . '}</style>';
	$button_classes[] = 'vc_btn-retro-btn-' . $uid;
} else {
...
?>
<div class="<?php echo trim( esc_attr( $css_class ) ) ?>"><?php 
if ( $use_link ) {
		echo '<a ' . $attributes . '>' . ( 'retro-txt' === $style ? '<span>' : '' ) . $button_html . ( 'retro-txt' === $style ? '</span>' : '' ) . '</a>';
} else {
	echo '<button ' . $attributes . '>' . ( 'retro-txt' === $style ? '<span>' : '' ) . $button_html . ( 'retro-txt' === $style ? '</span>' : '' ) . '</button>';
} ?></div>
