...
class Custom_CSS_Style
{
	...
	public function run() {
		...
		if ( $this->isCustomizeStyle() ) {
			...
			wp_enqueue_style( 'ox-custom-style', $uri,'', null, 'all' );
		} elseif ( is_multisite() ) {
			$uri = get_template_directory_uri() . '/css/style.php';
			wp_enqueue_style( 'ox-custom-style', $uri, '', null, 'all' );
	...
	private function getStyle() {
		...
		if ( get_option( SHORTNAME . '_gfontdisable' ) ) {
			if ( get_option( SHORTNAME . '_bazar_font' ) ) {
				$this->options_value['font_family'] = 'BazarMedium';
			} else {
				$this->options_value['font_family'] = get_option( SHORTNAME . '_fontfamily' );
			}
		} else // Google font enabled
		{

			$font_family = Admin_Theme_Element_Select_Gfont::compatibility( get_option( SHORTNAME . '_gfont' ) );
			$this->options_value['font_family'] = sprintf( "'%s'",$font_family[0]['family'] );
		}

			$rt_logo_font = Admin_Theme_Element_Select_Gfont::compatibility( get_option( SHORTNAME . '_logo_font' ) );
			$this->options_value['rt_logo_font'] = sprintf( "'%s'",$rt_logo_font[0]['family'] );

					/**
			 * file url hack
			 */
			$this->options_value['assets_url'] = get_template_directory_uri();
	...
	private function getTemplateStyleFilePath() {
		...
		return get_template_directory() . DIRECTORY_SEPARATOR
				 ...
				 . $this->getCurrentLayoutTemplateFile();
	}
	 ...
	 * @return type
	 */
	private function getCurrentLayoutTemplateFile() {

		$currentLayout = Admin_Theme_Menu_SkinGroup::getActiveLayout();

		switch ( $currentLayout ) {
			case '':
			case Admin_Theme_Menu_SkinGroup::NAME:
				return self::TEMPLATE_FILE;
				break;
			default :
				return sprintf( 'template.%s.css', $currentLayout );
		}
	}
	...
	function getMacrosesList() {
		...
		if ( $rt_admin_menu && $rt_admin_menu instanceof Admin_Theme_Menu ) {
			foreach ( $rt_admin_menu->getMenuPageList() as $adminPage ) {
	...
	public function sessionJSPreview() {
		...
		if ( $rt_admin_menu && $rt_admin_menu instanceof Admin_Theme_Menu ) {
			...
			foreach ( $rt_admin_menu->getMenuPageList() as $adminPage ) {
				if ( $adminPage && $adminPage instanceof Admin_Theme_Menu_Item ) {
					foreach ( $adminPage->getOptionsList() as $option ) {
						if ( $option 	&& ($option instanceof Admin_Theme_Element_Colorchooser
											|| $option->getId() == SHORTNAME . '_gfont'
											|| $option->getId() == SHORTNAME . Admin_Theme_Menu_SkinGroup::ACTIVE_SKIN_OPTION) ) {
