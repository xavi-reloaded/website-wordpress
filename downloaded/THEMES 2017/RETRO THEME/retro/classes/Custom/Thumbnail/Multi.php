...
class Custom_Thumbnail_Multi
{
	...
	public function add_attachment_field( $form_fields, $post ) {
		...
		if ( isset( $_GET['post_id'] ) ) {
			$calling_post_id = absint( $_GET['post_id'] ); } elseif ( isset( $_POST ) && count( $_POST ) ) { // Like for async-upload where $_GET['post_id'] isn't set
	...
	private function post_thumbnail_html( $thumbnail_id = null ) {
		...
		if ( version_compare( $wp_version, '3.5', '<' ) ) {
			...
			$image_library_url = esc_url_raw( add_query_arg( array( 'context' => $this->id, 'TB_iframe' => 1 ), remove_query_arg( 'TB_iframe', $image_library_url ) ) );
			$url_class = 'thickbox';
		} else {
		...
		if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
			...
			if ( ! isset( $_wp_additional_image_sizes[ "{$this->post_type}-{$this->id}-thumbnail" ] ) ) {
				$thumbnail_html = wp_get_attachment_image( $thumbnail_id, array( $content_width, $content_width ) ); } else {
