...
class Custom_MetaBox_Taxonomy
{
	...
	public function load_scripts_styles() {
		...
		if ( in_array( $taxnow, $this->_meta_box['pages'] ) ) {
			...
			wp_enqueue_style( 'th-admin-css', $plugin_path . '/backend/css/admin.css' );
			// Enqueue Meta Box Scripts
			wp_enqueue_script( 'tax-meta-clss', $plugin_path . '/backend/js/tax-meta-clss.js', array( 'jquery' ), null, true );
		}
	}
	...
	public function insert_images() {
		...
		foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
			...
			$li .= "<a title='" . __( 'Delete this image', 'retro' ) . "' class='at-delete-file' href='#' rel='{$nonce}|{$term_id}|{$id}|{$attachment_id}'><img src='" . $this->SelfPath . "/images/delete-16.png' alt='" . __( 'Delete', 'retro' ) . "' /></a>";
	...
	public function wp_ajax_delete_image() {
		...
		if ( $ok ) {
			...
			echo json_encode( array( 'message' => __( 'Cannot delete file. Something\'s wrong.', 'retro' ) ) );
			die();
		}
	}
	...
	public function show_field_repeater( $field, $meta ) {
		...
		if ( count( $meta ) > 0 && is_array( $meta ) ) {
			foreach ( $meta as $me ) {
				...
				echo '" alt="' . __( 'Remove', 'retro' ) . '" title="' . __( 'Remove', 'retro' ) . '" id="remove-' . $field['id'] . '"></div>';
				$c = $c + 1;
			}
		...
		echo '" alt="' . __( 'Add', 'retro' ) . '" title="' . __( 'Add', 'retro' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
	...
	public function show_field_file( $field, $meta ) {
		...
		if ( ! empty( $meta ) ) {
			$nonce = wp_create_nonce( 'at_ajax_delete' );
			echo '<div style="margin-bottom: 10px"><strong>' . __( 'Uploaded files', 'retro' ) . '</strong></div>';
			echo '<ol class="at-upload">';
			foreach ( $meta as $att ) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo '<li>' . wp_get_attachment_link( $att, '', false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . __( 'Delete', 'retro' ) . '</a>)</li>';
			}
			echo '</ol>';
		}
		...
		echo '<strong>' . __( 'Upload new files', 'retro' ) . '</strong>';
		echo '</div>';
		echo "<div class='new-files'>";
		echo "<div class='file-input'>";
		echo "<input type='file' name='{$field['id']}[]' />";
		echo '</div><!-- End .file-input -->';
		echo "<a class='at-add-file button' href='#'>" . __( 'Add more files', 'retro' ) . '</a>';
	...
	public function show_field_color( $field, $meta ) {
		...
		echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color', 'retro' ) . "'/>";
