...
abstract class Admin_Theme_Menu_Item
{
	 ...
	 */
	const NONCE_SAVE_ACTION = 'ox-save-action_nonce';

	/**
	 * Reset action name for wp_nonce_field && check_admin_referer
	 */
	const NONCE_RESET_ACTION = 'ox-reset-action_nonce';
	/**
	...
	public function renderPage() {
		...
		if ( $this->getMenuSlug() != SHORTNAME . '_dummy' ) {
			...
			$html .= '<div class="color_separator color_sep_topind"></div><div class="admin_bottom clearfix">';
			$html .= '<input name="save_options" type="submit" value="Save Changes" class="ox_save"  />' .
				$save_nonce
				. '</form>
			<form method="post" id="ox_reset"  action="">' .
				$reset_nonce
				. '<input type="hidden" name="reset_options" id="reset_options" /><input name="reset" type="submit" value="Reset" class="ox_reset" />
			</form>';
		}
	...
	public function addGroupedOptions( $group ) {
		if ( $group->isActive() || (defined( 'THEME_ACTIVATION_NOW' ) && THEME_ACTIVATION_NOW) ) {
			foreach ( $group->getGroupedOptionsList() as $option ) {
