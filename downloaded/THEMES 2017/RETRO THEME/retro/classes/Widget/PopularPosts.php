...
class Widget_PopularPosts extends Widget_Default implements Widget_Interface_Cache
{
				...
				<?php while ( $popular_posts->have_posts() ) : $popular_posts->the_post(); ?>
					<?php if ( $current_post_id != get_the_ID() && $shown_post <= $instance[ self::NUMBER ] ) :?>
						<li class="widget_post_area clearfix <?php if ( isset( $instance[ self::SHOW_IMAGE ] ) && $instance[ self::SHOW_IMAGE ] ) { echo('widget_post_thumbnail'); }?>">
							<?php if ( isset( $instance[ self::SHOW_IMAGE ] ) && $instance[ self::SHOW_IMAGE ] ) :?>
						...
						<?php endif;?>
							<div class="recent-txt-content"><a class="recent-title" href="<?php echo get_permalink( get_the_ID() ); ?>" title="<?php the_title(); ?>" ><?php the_title(); ?></a> <?php excerpt( 80 );?><?php if ( comments_open() ) { echo '<p>';
								comments_popup_link( __( '0 - Comments', 'retro' ), __( '1 - Comment', 'retro' ), __( '% - Comments', 'retro' ), 'comments' );
								echo '</p>';} ?></div>
						</li>
						<?php $shown_post++;
					endif;?>
	...
	private function getDefaultFieldValues() {
		...
		$default_values = array(
					self::TITLE		=> __( 'Popular post:', 'retro' ),
					self::NUMBER	=> 5,
