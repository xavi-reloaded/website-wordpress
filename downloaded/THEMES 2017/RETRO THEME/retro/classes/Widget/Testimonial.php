...
class Widget_Testimonial extends Widget_Default implements Widget_Interface_Cache
{
	...
	function form( $instance ) {
		$defaults = array(
			self::TITLE => __( 'Testimonials', 'retro' ),
			self::TIME => '10',
			self::CATEGORY => 'all',
			self::RANDOMIZE => '',
			self::EFFECT => 'fade',
		...
		<?php _e( 'Title:', 'retro' ); ?>
			...
			<p>
				<label for="<?php echo $this->get_field_id( self::CATEGORY ); ?>" >
					<?php _e( 'Category of testimonials:', 'retro' ); ?>
				</label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( self::TIME ); ?>"><?php _e( 'Number of second to show:', 'retro' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( self::RANDOMIZE ); ?>"><?php _e( 'Randomize testimonial:', 'retro' ); ?>
			...
			<p>
				<label for="<?php echo $this->get_field_id( self::EFFECT ); ?>" >
					<?php _e( 'Transition effect:', 'retro' ); ?>
				</label>
				<select name="<?php echo $this->get_field_name( self::EFFECT ); ?>" id="<?php echo $this->get_field_id( self::EFFECT ); ?>"  style="width:100%;">
					...
					foreach ( self::getEffectList() as $effect => $descr ) {
						...
						echo "<option $selected value='" . $effect . "'>" . $descr . '</option>';
					}
					?>
				</select>
			</p>
	...
	static function printWidgetJS( $widget_id, $effect, $randomize, $interval = 10 ) {
		...
		<?php endif; ?>
				...
				if (jQuery('#<?php echo $widget_id ?> .jcycle').length) {
					...
					jQuery('#<?php echo $widget_id ?> .jcycle').on('cycle-before', function(e, opts, curr, next) {
						animateHeight(next);
					});
				}

				function animateHeight(nextElement) {
