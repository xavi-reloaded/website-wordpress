...
class Envato_Theme_Backup
{
	...
	public function zip_archive() {
		...
		foreach ( $this->files() as $file ) {
			...
			if ( is_dir( trailingslashit( $this->root() ) . $file ) ) {
				$zip->addEmptyDir( trailingslashit( $file ) ); } elseif ( is_file( trailingslashit( $this->root() ) . $file ) ) {
	...
	public function check_archive() {
		...
		if ( ! file_exists( $this->archive_filepath() ) ) {
			$this->error( $this->archive_method(), __( 'The backup file was not created', 'retro' ) ); }
	...
	public function files() {
		...
		if ( ! empty( $this->unreadable_files ) ) {
			$this->warning( $this->archive_method(), __( 'The following files are unreadable and could not be backed up: ', 'retro' ) . implode( ', ', $this->unreadable_files ) ); }
	...
	private function errors_to_warnings( $context = null ) {
		...
		if ( $context ) {
			unset( $this->errors[ $context ] ); } else {
			$this->errors = array(); }
	}
...
function theme_envato_backup_pclzip_callback( $event, &$file ) {
	...
	if ( ! is_readable( $file['filename'] ) || ! file_exists( $file['filename'] ) ) {
		return false; } /* Match everything else past the exclude list */
