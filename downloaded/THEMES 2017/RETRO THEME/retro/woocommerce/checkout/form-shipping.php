...
<div class="woocommerce-shipping-fields">
	<?php if ( true === WC()->cart->needs_shipping_address() ) : ?>
		...
		<h2><?php _e( 'Shipping Details', 'retro' ); ?></h2>
		...
		<p class="form-row" id="ship-to-different-address">
			...
			<label for="ship-to-different-address-checkbox"
			       class="checkbox"><?php _e( 'Ship to a different address?', 'woocommerce' ); ?></label>
		</p>
		...
		<div class="shipping_address">
			...
			if ( version_compare( WC_VERSION, '3.0.0', '<' ) ) {
				$checkout_fields = $checkout->checkout_fields['shipping'];
			} else {
				$checkout_fields = $checkout->get_checkout_fields( 'shipping' );
			}
			?>
