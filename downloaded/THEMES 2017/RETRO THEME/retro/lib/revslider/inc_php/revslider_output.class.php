...
class RevSliderOutput{
	 ...
	 */
	private function getSlideFullWidthVideoData( $slide ) {
	...
	private function putSlides( $doWrapFromTemplate ) {
		...
		foreach ( $slides as $index => $slide ) {
			...
			*/
			...
			// Html
			echo "	<!-- SLIDE  -->\n";
			echo '  <li data-slidetitle="' . $slideTitle . '"  data-transition="' . $transition . '" data-slotamount="' . $slotAmount . '" ' . $add_rand . $htmlParams . $slide_title . ">\n";
			echo "		<!-- MAIN IMAGE -->\n";
	 ...
	 * put creative layer
	 */
	private function putCreativeLayer( $slide, $static_slide = false ) {
		...
		foreach ( $layers as $layer ) :
			...
			if ( $layer_2d_rotation == 0 ) {
				$outputClass = 'tp-caption ' . trim( $class ); } else {
				...
				switch ( $type ) {
					...
					case 'video':
						...
						if ( array_key_exists( 'autoplay', $videoData ) ) {
							$videoAutoplay = UniteFunctionsRev::getVal( $videoData, 'autoplay' ); } else { // backword compatability
							...
							if ( array_key_exists( 'autoplayonlyfirsttime', $videoData ) ) {
								$videoAutoplayOnlyFirstTime = UniteFunctionsRev::getVal( $videoData, 'autoplayonlyfirsttime' ); } else {
