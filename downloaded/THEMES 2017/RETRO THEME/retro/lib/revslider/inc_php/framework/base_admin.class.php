...
class UniteBaseAdminClassRev extends UniteBaseClassRev{
		...
		switch ( $contentType ) {
			...
			default:		// settings object
				...
				if ( ! empty( $drawFunction ) ) {
					call_user_func( $drawFunction,$output ); } else {
					$output->draw(); }
	...
	public static function addAdminMenu() {
		...
		foreach ( self::$arrMenuPages as $menu ) {
			...
			$revslider_screens[] = add_menu_page( $title, $title, $role, self::$dir_plugin, array( self::$t, $pageFunctionName ), get_template_directory_uri() . '/backend/img/i_slideshows.png', 8 );
		}
