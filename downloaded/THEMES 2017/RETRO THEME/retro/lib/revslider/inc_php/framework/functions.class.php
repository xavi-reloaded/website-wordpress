...
class UniteFunctionsRev{
	 ...
	 */
	public static function writeDebugError( $e, $filepath = 'debug.txt' ) {
	...
	public static function getHTMLSelect( $arr, $default = '', $htmlParams = '', $assoc = false ) {
		...
		foreach ( $arr as $key => $item ) {
			...
			} else { 				$html .= "<option $selected value='$item'>$item</option>"; }
		}
		$html .= '</select>';
		return($html);
	}
	...
	public static function copyDir( $source, $dest, $rel_path = '', $blackList = null ) {
		...
		foreach ( $files as $file ) {
			...
			if ( is_file( $path_source ) ) {
				...
				if ( empty( $rel_path ) ) {
					$rel_path_new = $file;
				} else { 					$rel_path_new = $rel_path . '/' . $file; }
