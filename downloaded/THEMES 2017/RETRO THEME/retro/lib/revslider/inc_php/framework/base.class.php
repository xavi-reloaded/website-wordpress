...
class UniteBaseClassRev{
	...
	public function __construct( $mainFile, $t ) {
		...
		self::$url_plugin = get_template_directory_uri() . '/lib/' . self::$dir_plugin . '/';
	...
	private static function setPathCache() {
		...
		$path = self::$path_plugin . 'cache/';
		$uploadsPath = wp_upload_dir();
		$path = $uploadsPath['basedir'] . '/cache/';

		UniteFunctionsRev::checkCreateDir( $path );
	...
	protected static function getPostGetVar( $key, $defaultValue = '' ) {
		...
		if ( array_key_exists( $key, $_POST ) ) {
			$val = self::getVar( $_POST, $key, $defaultValue ); } else {
			$val = self::getVar( $_GET, $key, $defaultValue ); }
