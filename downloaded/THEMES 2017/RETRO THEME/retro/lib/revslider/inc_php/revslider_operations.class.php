...
class RevOperations extends UniteElementsBaseRev{
	...
	public static function getCaptionsCssContentArray() {
		if ( file_exists( GlobalsRevSlider::$filepath_captions ) ) {
			$contentCSS = file_get_contents( GlobalsRevSlider::$filepath_captions ); } elseif ( file_exists( GlobalsRevSlider::$filepath_captions_original ) ) {
			$contentCSS = file_get_contents( GlobalsRevSlider::$filepath_captions_original ); } elseif ( file_exists( GlobalsRevSlider::$filepath_backup . 'captions.css' ) ) {
				$contentCSS = file_get_contents( GlobalsRevSlider::$filepath_backup . 'captions.css' ); } elseif ( file_exists( GlobalsRevSlider::$filepath_backup . 'captions-original.css' ) ) {
				$contentCSS = file_get_contents( GlobalsRevSlider::$filepath_backup . 'captions-original.css' );
				} else { 			UniteFunctionsRev::throwError( 'No captions.css found! This installation is incorrect, please make sure to reupload the Slider Revolution plugin and try again!' ); }
