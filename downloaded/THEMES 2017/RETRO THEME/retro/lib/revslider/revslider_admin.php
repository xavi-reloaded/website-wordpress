...
class RevSliderAdmin extends UniteBaseAdminClassRev{
	 ...
	 */
	public static function customPostFieldsOutput( $output ) {
		...
		try {
			...
			switch ( $action ) {
				...
				case 'delete_slide':
					...
					if ( $isPost ) {
						$message = __( 'Post Deleted Successfully',REVSLIDER_TEXTDOMAIN ); } else {
				...
				case 'preview_slider':
					...
					if ( $do_markup == 'true' ) {
						$operations->previewOutputMarkup( $sliderID ); } else {
