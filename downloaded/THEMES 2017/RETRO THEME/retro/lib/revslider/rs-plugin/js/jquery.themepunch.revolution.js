...
(function (jQuery, undefined) {
	...
	jQuery.fn.extend({
		...
		revolution: function (options) {
			...
			var defaults = {
				...
				thumbHeight: 50,
				thumbAmount: 3,
				maxVisibleBullets: 5, //@custom
				bulletStep: 118, //@custom
				navigationType: "bullet", // bullet, thumb, none
				navigationArrows: "nexttobullets", // nextto, solo, none
	...
	function initSlider(container, opt) {
		...
		if (!container.hasClass("revslider-initialised")) {
			 ...
			 */
			if (
					opt.slideamount > opt.maxVisibleBullets
					&& opt.navigationType != 'none'					
				)
			{
			...
			jQuery('#unvisible_button').click(function () {
				...
				opt.navigationArrows = 'nexttobullets';
...
//							if (opt.slideamount >1) createArrows(container,opt);
	 ...
	 * @custom
	 */
	function moveBullets(container, opt) {
		if (opt.slideamount <= opt.maxVisibleBullets)
			return;
		var $bullets = container.parent().find('.tp-bullets span'),
				$selected = $bullets.find('.selected')
		$bullets.find('.bullet').hide();
		$selected.show();
	}
	...
	var paginTimeLine = {
		...
		start: function (duration) {
			if (this.timelines.length > 0) {
				this.timeline.animate({'width': "100%"}, {duration: duration, queue: false, easing: "linear"});
			}
		},
		stop: function (container) {
			this.timeline = container.parent().find('.tp-bullets .bullet.selected').next().find('div');
			this.timeline.stop();
		},
		restart: function (container, duration) {
			this.timelines = this.timelines || container.parent().find('.tp-bullets .separator div');
			this.timelines.each(function () {
	...
	var createBullets = function (container, opt) {
		...
		if (opt.navigationType == "bullet" || opt.navigationType == "both") {
			container.parent().append('<div class="tp-bullets ' + starthidebullets + ' simplebullets ' + opt.navigationStyle + '"><span></span></div>'); // @custom
		}

		//@custom
		var bullets = container.parent().find('.tp-bullets span'),
				slides = container.find('>ul:first >li'),
				index = 0,
				bullet,
				width,
				count = slides.length;
...
//		});
		...
		slides.each(function (i) {
			i = i + 1;
			var src = container.find(">ul:first >li:eq(" + i + ") img:first").attr('src'),
					title = jQuery(this).data('slidetitle');
			index = i;
			index = (opt.slideamount > opt.maxVisibleBullets) ? index + '/' + opt.slideamount : index;
			bullets.append('<div class="bullet" data-id="' + (i) + '"><div class="bullet-custom">' + index + '</div><div class="slidetitle">' + title + '</div></div>');
			if (opt.slideamount <= opt.maxVisibleBullets && --count != 0)
				bullets.append('<div class="separator"><div></div></div>');
		});

		bullet = bullets.find('.bullet:first')
...
//				if (opt.navigationArrows == "withbullet" || opt.navigationArrows == "nexttobullets") {
					if (bul.data('id') - 1 == opt.act)
						sameslide = true;
//				} else {
//					if (bul.data('id') == opt.act)
//						sameslide = true;
//				}
...
//					if (opt.navigationArrows == "withbullet" || opt.navigationArrows == "nexttobullets") {
						opt.next = bul.data('id') - 1;
//					} else {
//						opt.next = bul.data('id');
//					}
...
//			arst = opt.soloArrowStyle;
		...
		function aArrow(dir) {
			container.parent().append('<div style="' + hidden + '" class="tp-' + dir + 'arrow ' + starthidearrows + ' tparrows ' + arst + '"><div class="tparrows-inner"></div></div>'); //@custom
		}
		aArrow("left");
		aArrow("right");
...
//			opt.navigationArrows = "solo";
		...
		if (opt.navigationType !== "thumb") {
			tl.prependTo(bullets.find('span')).css({'float': 'left'});//@custom
...
//			if (opt.navigationArrows == "withbullet" || opt.navigationArrows == "nexttobullets") {
				if (bul.data('id') - 1 == opt.next)
					bul.addClass('selected');

//			} else {
//
//				if (bul.data('id') == opt.next)
//					bul.addClass('selected');
//
//			}
		});
			...
			container.hover(
					...
					function () {
						if (container.data('conthover') != 1) {
							 ...
							 * @custom
							 */
							paginTimeLine.start((opt.delay - opt.cd) - 100);
							container.trigger('revolution.slide.onresume');
							container.trigger('starttimer');
