...
FB.ContactForm = function() {
	...
	'action' : 'ox_contactform_shortcode',
	...
	function sendShortcodeToEditor () {
		try {
			...
			jQuery.post(ajaxurl, fbForm, function(response) {
				...
				var regexp = new RegExp("\\[ox-contact-form\\b.*?\\/?\\](?:[\\s\\S]+?\\[\\/ox-contact-form\\])?");
...
//				jQuery('#fb-field-my-email').focus().select();
	...
	function updateUseCaptcha()
	{
		try{
			fbForm.captcha = jQuery('#fb-captcha').is(':checked');
		}
		catch (e) {
			if (debug) {
				console.log("updateUseCaptcha(): " + e);
			}
		}
	}
...
//			}
//			var data = {
//				action: 'ox_contactform_shortcode_to_json',
...
//			});
			...
			jQuery('#fb-captcha').change(function(){
				updateUseCaptcha();
			});
