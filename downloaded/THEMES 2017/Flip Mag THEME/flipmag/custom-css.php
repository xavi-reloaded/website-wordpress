...
/**
 * Render the CSS customizations using Flipmag Framework options
 */
class Flipmag_Custom_Css
{
	public $elements;
	public $css_options;
	...
	public function init()
	{
		...
		$admin_options = Flipmag::factory('admin/options'); /* @var $admin_options Flipmag_Admin_Options */
		$this->elements = $admin_options->get_elements_from_tree($options);
		
		// get all css options
		$this->css_options = Flipmag::options()->get_all('css_');
		...
		foreach ($this->css_options as $key => $value)
		{
			...
			else if (isset($element['css']) && is_array($element['css']['selectors'])) 
			{
				foreach ($element['css']['selectors'] as $css_key => $format)
				{
					...
					if (Flipmag::options()->oc_no_responsive && strstr($css_key, '@media')) {
						continue;
					}
	...
	{	
		...
		if ($google_fonts) {
			...
			if (Flipmag::options()->font_charset) {
				$charsets = implode(',', array_keys(array_filter(Flipmag::options()->font_charset)));
			}
...
EOF;
		...
		$cat_css = apply_filters('flipmag_css_generator_cat', $cat_css);
		$anchors = array();
		...
		foreach ((array) Flipmag::options()->get_all('cat_meta_') as $key => $cat) {
			...
			if (!empty($cat['main_color']) && in_array($key, $anchors)) {
				...
				foreach ($rules as $css_key => $format) {
					...
					// ignore media querie selectors if responsive is disabled
					if (Flipmag::options()->no_responsive && strstr($css_key, '@media')) {
						continue;
					}
	...
	public function render()
	{
		...
		// have data in cache?
		$cache = get_transient('flipmag_custom_css_cache');
		...
		// cache it
		set_transient('flipmag_custom_css_cache', array($anchor => $output));
