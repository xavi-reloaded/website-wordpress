...
class Flipmag_Menu_Walker extends Walker_Nav_Menu
{
	...
	public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) 
	{
		if ($item->object == 'category') {
			$item->classes = array_merge((array) $item->classes, array('menu-cat-' . $item->object_id)); 
                        $item->classes = array_merge((array) $item->classes, array('mega-cat'));                         
		}
		...
		if ($item->mega_menu) {
			...
			$this->current_item = $item;
                ...
                if( $this->in_mega_menu && $depth > 0 && $item->object == 'category' ){                    
                    $item_output = str_replace('<li ','<li data-cat-id="'. $item->object_id .'" ', $item_output );
                }
	...
	{	
		...
		if ($this->in_mega_menu) 
		{
			...
			if ($depth == 0) {
				$output .= apply_filters('flipmag_mega_menu_end_lvl', array('sub_menu' => $this->last_lvl, 'item' => $this->current_item));
