...
class Flipmag_Admin_MetaBoxes
{
	private $prefix;
	private $option_prefix = '_flipmag_';  // _ underscore hides in custom fields
	...
	public function __construct()
	{
		...
		$this->prefix        = Flipmag::options()->get_config('theme_prefix') . '_';
		$this->option_prefix = Flipmag::options()->get_config('meta_prefix') . '_';
	}
	...
	public function init()
	{
		...
		$meta = Flipmag::options()->get_config('meta_boxes');
		if (!is_array($meta)) {
			return;
		}
	...
	{
		$meta = (array) Flipmag::options()->get_config('meta_boxes');
		foreach ($meta as $box) {
	...
	public function render($post = null, $args = null)
	{
		...
		Flipmag::factory('admin/option-renderer'); // load 
		
		$file = sanitize_file_name(str_replace($this->prefix, '', $args['id'])) . '.php';
		$meta = Flipmag::factory('admin/meta-renderer', true); /* @var $meta Flipmag_Admin_MetaRenderer */

		// render the template
		$meta->set_prefix($this->option_prefix)->template(
			array(),
			locate_template('admin/meta/' . $file),
			Flipmag::posts()->meta(null, $post->ID), // populate all existing meta
