...
class Flipmag_Admin_OptionRenderer
{
	...
	public function render($element)
	{
		...
		{
			case 'posts':
				$output = $this->render_posts($element);
				break;

            case 'info':
				$output = $this->render_info($element);
				break;
                            
            case 'images':
				$output = $this->render_images($element);
				break;
		...
		if ($output && empty($element['no_wrap'])) {
			$output = '<label class="element-title">'. wp_kses_stripslashes($element['label']) . '</label>'
					. '<div class="element-control">' . wp_kses_stripslashes($output) . (isset($element['html_post_output']) ? $element['html_post_output'] : '') . '</div>';
		}
		
		return $output;
	}

	public function render_posts($element)
	{
		$output = '';

		if( class_exists('SiteOrigin_Widgets_Bundle') ){
			$output .= '<input type="hidden" value="'. esc_attr($element['value']) .'" name="'. esc_attr($element['name']) .'" class="siteorigin-widget-input" />';
			$output .= '<a href="#" class="sow-select-posts button button-primary">';
			$output .= esc_html__( 'Build posts query', 'flipmag' );
			$output .= '</a>';
		}
		return $output ;
	}
        ...
        public function render_images($element)
	{
		...
		{
                    $active = "";
                    if( $element['value'] == $key ){ $active = 'active'; }
                    
                    $rand = rand( 10000, 100000)."_".rand( 10000, 100000)."_".rand( 10000, 100000);
                    $output .= '<div class="radio-images '. esc_attr($element['name']) .' '. esc_attr($active) .' "  id="radio-images-select-'. esc_attr($rand) . '-'. esc_attr($key) .'" ><label><input type="radio" style="position:absolute; left: -9999px;" name="'. esc_attr($element['name']) .'" value="'. esc_attr($key) . '"'
                            .checked($element['value'], $key, false) .' id="images-select-'. esc_attr($rand) . '-'. esc_attr($key) .'" /><img src="' . esc_url($option) . '" ></label></div>';
		}
	...
	public function _render_checkbox($element) 
		...
		$element['options'] = array_merge(
			array('checked' => __('Yes', 'flipmag'), 'unchecked' => __('No', 'flipmag')), 
		...
		$output = '<input type="hidden" name="'. esc_attr($element['name']) .'" value="0" />' // always send in POST - even when empty
				...
				<label for="'. esc_attr($element['name']) .'">' . wp_kses_stripslashes($element['label']) . '</label>
				';
	...
	public function render_textarea($element)
	{
		...
		if ($element['options']['rows'] OR $element['options']['cols']) {
			$row_cols = ' rows="' . esc_attr(intval($element['options']['rows'])) . '" cols="' . esc_attr($element['options']['cols']) . '"'; 
		}
	 ...
	 * 
	 * @param array $element
	 * @uses Flipmag_Admin_OptionRenderer::render_upload()
	 */
	public function render_bg_image($element)
	{
        ...
        public function render_info($element)
	{
            $output = '';
		...
		{ 
			...
			if (isset($element['bg_type'])) 
			{
				...
				$type .= $this->render_select(array(
					...
					'options' => array(
						'repeat' => __('Repeat Horizontal and Vertical - Pattern', 'flipmag'),
						'cover'  => __('Fully Cover Background - Photo', 'flipmag'),
						'repeat-x' => __('Repeat Horizontal', 'flipmag'),
						'repeat-y' => __('Repeat Vertical', 'flipmag'),
						'no-repeat' => __('No Repeat', 'flipmag'),
					),
				));
			}
			
			$output .= '<a href="" class="remove-image button after-upload'. esc_attr($classes) .'">'. __('Remove', 'flipmag') .'</a>';			
			$output .= '<div class="image-upload'. esc_attr($classes) .'">'. $image . '</div>';
			
			$output .= '<div class="image-type after-upload'. esc_attr($classes) .'">' . $type . '</div>';
		}
	...
	public function render_typography($element)
	{
		...
		$stacks = apply_filters('flipmag_options_font_stacks', array(
		...
		if (is_array($element['suggested'])) {
			$options[__('Suggested Fonts', 'flipmag')] = $element['suggested'];
		}
		
		$options[__('Google Fonts', 'flipmag')] = $google;
		$options[__('System Fonts', 'flipmag')] = $stacks; 
	...
	{
		$fonts = wp_remote_retrieve_body( wp_remote_get(get_template_directory() .'/admin/fonts.json'));
