<?php
class Flipmag_Menu_Edit_Walker extends Walker_Nav_Menu_Edit 
{
	...
	{
		$fields = apply_filters('flipmag_custom_menu_fields', array());
		$output = '';
		...
		foreach ($fields as $key => $field)
		{
			...
			$output .= '<p class="field-custom description description-wide">
				<label for="edit-menu-item-subtitle-'. $item->ID .'">'. esc_html($field['label']) .'<br />'. wp_kses_stripslashes($template)  .'
				</label>
			</p>';
		}
