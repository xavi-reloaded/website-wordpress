...
/**
 * Flipmag Framework - factory and pseudo-registry for objects
 * 
...
class Flipmag {
	...
	public static function factory($object = 'core', $fresh = false)
	{
		...
		if (!empty(self::$_registered[$object]['class'])) {
			...
			// convert short-codes to Flipmag_ShortCodes; core to Flipmag_Core etc.
			$class = str_replace('/', '_', $object);
			$class = apply_filters('flipmag_factory_class', 'Flipmag_' . self::file_to_class_name($class));
		}
	 ...
	 * @return Flipmag_Posts
	 */
	public static function blocks($fresh = false)
	{
		return self::factory('blocks', $fresh);
	}
	
	/**
	 * WordPress Menus related functionality
	 * 
	 * @return Flipmag_Menus
	 */
