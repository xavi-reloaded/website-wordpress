...
/**
 * Modified for Flipmag Framework - added dependencies (functions)
 */
...
function wie_upload_import_file() {
	...
	if ( ! empty( $_POST ) && ! empty( $_FILES['wie_import_file'] ) && check_admin_referer( 'wie_import', 'wie_import_nonce' ) ) { // check_admin_referer prints fail page and dies
		...
		if ( 'wie' != $wp_filetype['ext'] && ! wp_match_mime_types( 'wie', $wp_filetype['type'] ) ) {
			wp_die(
				sprintf( __( 'You must upload a %s .wie %s file generated by this plugin.', 'flipmag' ), '<b>', '</b>' ),
			'',
...
function wie_process_import_file( $file ) {
	...
	if ( ! file_exists( $file ) ) {
		wp_die(
			__( 'Import file could not be found. Please try again.', 'flipmag' ),
			'',
			array( 'back_link' => true )
		);
	}
...
function wie_import_data( $data ) {
	...
	if ( empty( $data ) || ! is_object( $data ) ) {
		wp_die(
			__( 'Import data could not be read. Please try a different file.', 'flipmag' ),
			'',
			array( 'back_link' => true )
		);
	}
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
			...
			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'flipmag' );
		}
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = __( 'Site does not support widget', 'flipmag' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = __( 'Widget already exists', 'flipmag' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = __( 'Imported', 'flipmag' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = __( 'Imported to Inactive', 'flipmag' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'flipmag' ); // show "No Title" if widget instance is untitled
