...
class Flipmag_Posts
{
	...
	public function the_content($more_link_text = null, $strip_teaser = false)
	{
		...
		// apply flipmag_main_content filters first - for page builder
		$content = apply_filters('the_content',  $content, 'flipmag_main_content');
	...
	{
		$prefix = Flipmag::options()->get_config('meta_prefix') . '_';
		...
		if (is_string($key)) {
			...
			if ($defaults) {
				...
				$default_map = array('featured_disable' => array('key' => 'oc_show_featured', 'bool_inverse' => true));
				
				// have a key association with theme settings?
				if (!$meta && array_key_exists($key, $default_map)) {
					
					$expression = Flipmag::options()->get($default_map[$key]['key']);
										
					return ($default_map[$key]['bool_inverse'] ? !$expression : $expression);
				}
			}
			
			return apply_filters('flipmag_meta_' . $key, $meta);
	...
	{	
		...
		if (Flipmag::options()->oc_related_posts_yarpp && function_exists('yarpp_get_related')) {
		...
		if (Flipmag::options()->oc_related_posts_by == 'tags') {
		...
		$related = get_posts(apply_filters('flipmag_get_related_query', $args));
	...
	public function paginate($options = array(), $query = null)
	{
		...
		$args = array(
			...
			'next_text' => '<span class="visuallyhidden">' . _x('Next', 'pagination', 'flipmag') . '</span><i class="fa fa-angle-right"></i>',
			'prev_text' => '<i class="fa fa-angle-left"></i><span class="visuallyhidden">' . _x('Previous', 'pagination', 'flipmag') . '</span>'
		);
	 ...
	 * @see paginate_links()
	 */
	public function ajaxPaginate($options = array(), $query = null)
	{
	...
	public function excerpt_read_more()
	{
        ...
        return '<div class="read-more"><a href="'. esc_url(get_permalink($post->ID)). '" title="'. esc_attr($text) . '">'. $text .'</a></div>';	
	}
}