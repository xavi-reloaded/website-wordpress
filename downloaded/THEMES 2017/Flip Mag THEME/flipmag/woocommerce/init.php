...
class FliptMag_WooCommerce
{
	public function __construct()
	{
		...
		add_filter('flipmag_theme_options', array($this, 'add_theme_options'));
	...
	public function init()
	{
		...
		// add menu cart item
		if (Flipmag::options()->woocommerce_nav_cart) {
		...
		// related post count
		add_filter('woocommerce_output_related_products_args', array($this, 'oc_related_posts'));
		...
		add_filter('flipmag_metabox_page_options', array($this, 'remove_page_options'));
		
		// per page settings?
		if (Flipmag::options()->woocommerce_per_page) {
			add_filter(
				'loop_shop_per_page', 
				create_function('$cols', 'return ' . intval(Flipmag::options()->woocommerce_per_page) . ';'), 
				20
			);
		}
	}
	...
	public function set_sidebar() 
	{
		...
		if (is_woocommerce() && !is_product()) {
			$layout = Flipmag::posts()->meta('layout_style', wc_get_page_id('shop'));
		}
		...
		if ($layout) {
			Flipmag::core()->set_sidebar(($layout == 'full' ? 'none' : $layout));
		}
		else { 
			// use default sidebar setting for WooCommerce
			Flipmag::core()->set_sidebar(Flipmag::options()->woocommerce_sidebar);
		}
	}
	...
	{
		wp_enqueue_style('flipmag-woocommerce', get_template_directory_uri() . '/css/' . (is_rtl() ? 'rtl-' : '') . 'woocommerce.css'); 
	}
	...
	public function comment_form($args)
	{
		...
		$args = array_merge($args, array(
			'title_reply'    => '<span class="section-head">' . (have_comments() ? _x('Add a review', 'woocommerce', 'flipmag') : _x('Be the first to review', 'woocommerce', 'flipmag') . ' &ldquo;' . get_the_title() . '&rdquo;') . '</span>',
			'title_reply_to' => '<span class="section-head">' . _x( 'Leave a Reply to %s', 'woocommerce', 'flipmag') . '</span>',
			/*'fields'  => array(
	 ...
	 */
	public function oc_related_posts($args)
	{
		$args = array_merge($args, array(
			'posts_per_page' => (Flipmag::core()->get_sidebar() == 'none' ? 4 : 3),
			'columns' => (Flipmag::core()->get_sidebar() == 'none' ? 4 : 3),
		));
	...
	public function output_upsells() 
	{
		...
		$number = Flipmag::core()->get_sidebar() == 'none' ? 4 : 3;
	...
	{		
		return (Flipmag::core()->get_sidebar() == 'none' ? 4 : 3);
	}
	...
	{		
    	...
    	?>
        	<li class="shopping-cart menu-item menu-item-type-custom menu-item-object-custom">
        		...
        		<?php the_widget('WC_Widget_Cart', 'title= ', array('before_widget' => '<div class="mega-menu cart-widget widget_shopping_cart">', 'before_title' => '', 'after_title' => '' )); ?>        		
        	</li>
        <?php
	...
	public function cart_menu_link()
	{
		...
		ob_start();
		?>
			<a href="<?php echo esc_url($woocommerce->cart->get_cart_url() ); ?>" class="cart-link"><i class="fa fa-shopping-cart"></i>
				<span class="counter <?php echo esc_attr($count ? ' active' : ''); ?>"><?php echo esc_html($count); ?></span>
				<span class="text"><?php _e('Shopping Cart', 'flipmag'); ?></span></a>		
		<?php
		return ob_get_clean();
	}
				...
				array(
					'fields' => array(
						array(
							...
							'label'   => __('WooCommerce Default Sidebar', 'flipmag'),
							'value'   => 'none',
							'desc'    => __('Specify the sidebar to use by default on WooCommerce listings and pages. This can be overriden per-page except for single page product.', 'flipmag'),
							'type'    => 'radio',
						...
						array(
							...
							'label'   => __('Posts Per Page', 'flipmag'),
							'value'   => '',
							'desc'    => __('The number of posts to show per page on WooCommerce shop listing. Leave empty to use default WordPress posts per page settings.', 'flipmag'),
							'type'    => 'number',
						),
						
						array(
							'name' 	  => 'woocommerce_nav_cart',
							'label'   => __('Add Cart In Navigation', 'flipmag'),
							'value'   => 1,
							'desc'    => __('When enabled, a cart icon is shown in the navigation to the right side.', 'flipmag'),
							'type'    => 'checkbox',
						),
					)
				)
