...
function presscore_add_bulk_edit_fields( $col, $type ) {
	...
	<?php if ( in_array( $type, array( 'post', 'dt_portfolio' ) ) ) : ?>
	...
	if ( isset($_REQUEST['bulk_edit']) ) {
		...
		if ( isset( $_REQUEST['_dt_bulk_edit_show_thumbnail'] ) && '-1' != $_REQUEST['_dt_bulk_edit_show_thumbnail'] ) {
		    $post_type_meta = array(
                'post' => '_dt_post_options_hide_thumbnail',
                'dt_portfolio' => '_dt_project_options_hide_thumbnail',
            );

		    if ( array_key_exists( $post->post_type, $post_type_meta ) ) {
