...
if ( pixelgrade_option('header_width') == 'content' ) {
	$header_width = pixelgrade_option('content_width') . 'px';
} else {
	$header_width = '100%';
	$header_classes .= ' header-padding';
}

$header_classes .= pixelgrade_option('header_position') == 'static' ? ' site-header--static' : '';
$header_classes .= pixelgrade_option('nav_header_style') == 'trigger' ? ' site-header--trigger' : '';
$header_classes .= pixelgrade_option('header_transparent') == true && ! ( class_exists( 'WooCommerce' ) && is_product() ) ? ' js-transparent-header site-header--transparent' : '';
?>
...
<header class="site-header  header-height <?php echo $header_classes; ?>">
	...
	<div class="panel">
		...
		<a class="navigation-toggle  js-nav-toggle">
			<?php if ( pixelgrade_option('nav_menu_layout') == 'text' || pixelgrade_option('nav_menu_layout') == 'text-icon' ) {
				echo '<span>'.pixelgrade_option('nav_menu_text').'</span>';
			}
			if ( pixelgrade_option('nav_menu_layout') == 'icon' || pixelgrade_option('nav_menu_layout') == 'text-icon' ) {
...
<?php if ( pixelgrade_option( 'enable_copyright_overlay' ) ) : ?>

<div class="copyright-overlay">
	<div class="copyright-overlay__container">
		<div class="copyright-overlay__content">
			<?php echo pixelgrade_option( 'copyright_overlay_text' ) ?>
		</div>
	</div>
</div>
