...
function pile_load_woocommerce_assets() {
	...
	if ( ! pixelgrade_option( 'enable_woocommerce_support' ) ) {
		return;
	}
...
function pile_localize_ajax_ignored_woo_links( $pile_non_ajax_links ) {
	...
	return $pile_non_ajax_links;
}
add_filter( 'pile_ajax_ignored_links', 'pile_localize_ajax_ignored_woo_links' );
...
if ( ! function_exists( 'pile_checkout_coupon_form' ) ) {
	function pile_checkout_coupon_form() {
		echo '<form id="pile-checkout-coupon-form" class="checkout_coupon" hidden></form>';
	}
}
...
if ( ! function_exists( 'pile_checkout_coupon_form_fields' ) ) {
	function pile_checkout_coupon_form_fields() {
		if ( wc_coupons_enabled() ) {
			echo apply_filters( 'woocommerce_checkout_coupon_message', esc_html__( 'Have a coupon?', 'woocommerce' ) . ' <a href="#" class="showcoupon">' . esc_html__( 'Click here to enter your code', 'woocommerce' ) . '</a>' ); ?>
			<div class="checkout_coupon" method="post" style="display:none">
				<p class="form-row form-row-first">
					<input type="text" name="coupon_code" form="pile-checkout-coupon-form" class="input-text"
					       placeholder="<?php esc_attr_e( 'Coupon code', 'woocommerce' ); ?>" id="coupon_code"
					       value=""/>
				</p>
				<p class="form-row form-row-last">
					<input type="submit" class="button" form="pile-checkout-coupon-form" name="apply_coupon"
...
if ( ! function_exists( 'pile_enquiry_text' ) ) {
	function pile_enquiry_text() {
		$enquiry_text = pixelgrade_option( 'product_enquiry_text' );
...
if ( ! function_exists( 'pile_product_share' ) ) {
	function pile_product_share() {
		$product_single_show_share_links = pixelgrade_option( 'product_single_show_share_links' );
 ...
 * @return mixed
 */
function pile_woo_add_notice_classes( $message, $products ) {
...
add_filter( 'wc_add_to_cart_message_html', 'pile_woo_add_notice_classes', 10, 2 );
...
function pile_product_portfolio_3d_classes( $classes ) {
	...
	if ( is_shop() || is_product_taxonomy() ) {
		...
		$products_pile_3d_effect = pixelgrade_option( 'products_pile_3d_effect' );
		// Should we apply a drop-dead-when-you-see-it 3D effect?
		if ( empty( $products_pile_3d_effect ) ) {
			$classes[] = 'pile--no-3d';
		} else {
			$classes[] = ' pile--' . pixelgrade_option( 'products_pile_3d_target_rule' );
			$classes[] = ' pile--' . pixelgrade_option( 'products_pile_3d_target' );
		}
	}
...
function pile_products_column_sizes_classes( $classes ) {
	...
	if ( is_shop() || is_product_taxonomy() ) {
		$large_no = pixelgrade_option( 'products_pile_large_columns' );
		...
		$medium_no = pixelgrade_option( 'products_pile_medium_columns' );
		//apply the default if nothing else
		if ( empty( $medium_no ) ) {
			$medium_no = 2;
		}
		...
		$small_no = pixelgrade_option( 'products_pile_small_columns' );
		//apply the default if nothing else
		if ( empty( $small_no ) ) {
			$small_no = 1;
		}
