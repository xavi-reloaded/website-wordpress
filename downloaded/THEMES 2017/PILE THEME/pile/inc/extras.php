...
function pile_body_classes( $classes ) {
	if ( pixelgrade_option( 'enable_copyright_overlay' ) ) {
...
function pile_portfolio_3d_classes( $classes ) {
	...
	$pile_3d_effect = pixelgrade_option( 'pile_3d_effect' );
	if ( empty( $pile_3d_effect ) ) {
		$classes[] = 'pile--no-3d';
	} else {
		$classes[] = 'pile--' . pixelgrade_option( 'pile_3d_target_rule' );
		$classes[] = ' pile--' . pixelgrade_option( 'pile_3d_target' );
	}
...
function pile_portfolio_column_sizes_classes( $classes ) {
	$large_no  = pixelgrade_option( 'pile_large_columns' );
	...
	$medium_no = pixelgrade_option( 'pile_medium_columns' );
	//apply the default if nothing else
	if ( empty( $medium_no ) ) {
		$medium_no = 2;
	}
	...
	$small_no  = pixelgrade_option( 'pile_small_columns' );
	//apply the default if nothing else
	if ( empty( $small_no ) ) {
		$small_no = 1;
	}
...
function pile_portfolio_infinite_scroll_class( $classes ) {
	...
	if ( pile_toBool( $infinite_scroll ) ) {
...
function pile_option( $option, $default = null ) {
	_deprecated_function('pile_option', '2.2.0', 'pixelgrade_option');
...
function pile_callback_addthis() {
	...
	$social_share = false;
	if ( is_singular() && ( pixelgrade_option( 'blog_single_show_share_links' ) || pixelgrade_option( 'portfolio_single_show_share_links' ) ) ) {
		$social_share = true;
	}
	if ( is_page() ) {
...
<script type="text/javascript">
    ...
    <?php
    $addthis_variable = array(
        'url' => get_permalink(),
        'title' => wp_title( '|', false, 'right' ),
        'description' => trim( strip_tags( get_the_excerpt() ) )
    );
            echo 'addthis_share = ' . json_encode( $addthis_variable );
    ?>
 ...
 */
function pile_wrap_content_portfolio_before( $pile_item_index, $post_id = 0 ) {
	// Extra post classes
	$classes = array( 'pile-item', 'pile-item--archive', 'one-whole', 'lap-one-half', 'desk-one-third' );

	$column =  pixelgrade_option( 'pile_3d_target' );
	$large_no  = pixelgrade_option( 'pile_large_columns' );
	$medium_no = pixelgrade_option( 'pile_medium_columns' );
	$small_no  = pixelgrade_option( 'pile_small_columns' );
	...
	<div id="post-<?php echo $post_id; ?>" <?php post_class( $classes ); ?>>
...
function pile_load_next_projects() {
	...
	if ( ! empty( $posts ) ) {
		...
		foreach ( $posts as $post ) : setup_postdata( $post );
			...
			pile_wrap_content_portfolio_before( $pile_item_index, $post->ID );
...
function pile_create_project_shortcode( $atts ) {
	...
	if ( ! empty( $id ) && intval( $id ) ) {
		// we need a custom query
		$query = new WP_Query( array( 'page_id' => intval( $id ) ) );
		if ( $query->have_posts() ) {
			$query->the_post();
		}
	}
	...
	if ( in_array( 'next', $atts ) || in_array( 'Next', $atts ) ) {
		...
		$output .= ob_get_clean();
	} elseif ( in_array( 'categories', $atts ) || in_array( 'Categories', $atts ) ) {
		ob_start();
		get_template_part( 'templates/shortcodes/project/categories' );
		$output .= ob_get_clean();
	} elseif ( in_array( 'title', $atts ) || in_array( 'Title', $atts ) ) {
		$output .= get_the_title();
	}

	// reset the post data, just in case
	wp_reset_postdata();
...
if ( ! function_exists( 'pile_show_mini_cart' ) ) {
	function pile_show_mini_cart() {
		return class_exists( 'WooCommerce' ) && ( is_woocommerce() || is_shop() || is_cart() || is_checkout() ) && pixelgrade_option( 'show_cart_menu' );
	}
}

// add woocommerce urls to djax ignore list
function pile_localize_ajax_ignored_links() {
	...
	$pile_non_ajax_links = apply_filters( 'pile_ajax_ignored_links', $pile_non_ajax_links );

	if ( ! empty ( $pile_non_ajax_links ) ) {
		wp_localize_script( 'pile-main-scripts', 'pile_non_ajax_links', $pile_non_ajax_links );
	}
}
add_action( 'wp_enqueue_scripts', 'pile_localize_ajax_ignored_links', 20 );