...
function pile_get_hero_description( $post = null, $link_project = false ) {
	...
	// get the position set for the content of this slide(s)
	$hero_desc_position = get_post_meta( get_the_ID(), '_hero_description_alignment', true );
	?>

	<div class="hero<?php echo '  ' . $hero_desc_position; ?>">
...
function pile_the_hero_video ($id = null, $opacity = 100, $ignore_video = false ) {
	...
	if ( false !== strpos( $mime_type, 'video' ) ) {
		...
		echo '<div class="video-placeholder" data-src="' . wp_get_attachment_url( $attachment->ID ) . '" data-poster="' . $image . '" ' . $opacity . '"></div>';
...
if ( ! function_exists('pile_body_attributes') ) :
	...
	function pile_body_attributes() {
		$data_ajaxloading     = ( pixelgrade_option( 'use_ajax_loading' ) ) ? 'data-ajaxloading=""' : '';
		...
		if ( class_exists( 'WooCommerce' ) && ( is_shop() || is_product_taxonomy() ) ) {
			$data_parallax_amount = 'data-parallax="' . pixelgrade_option( 'products_parallax_amount' ) . '"';
		} else {
			$data_parallax_amount = 'data-parallax="' . pixelgrade_option( 'parallax_amount' ) . '"';
		}
