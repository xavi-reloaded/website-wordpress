(function($){
			...
			featured_image.on('html-change-post', function() {
				...
				if ( image.length > 0 ) {
					...
					$.ajax({
						type: "post",
						url: ajaxurl,
						data: { action: 'pile_get_project_color', attachment_src: src },
	...
	$(window).load(function () {
		...
		var check_number_of_slides = function() {
			...
			if ( $('#_pile_portfolio_featured_projects').parents('.cmb-type').is(':visible') && '' !== $('#_pile_portfolio_featured_projects').val() ) {
				featured = $('#_pile_portfolio_featured_projects').val().split(',').length;
			}
		...
		var has_cover_description_content = function() {
			if( typeof (tinyMCE) === "undefined" ) {
				return !!$('#_pile_header_cover_description').val();
			}

			var hero_editor = tinyMCE.get('_pile_header_cover_description');
		...
		var check_hero_desc_visibility = function () {
			...
			$('#wp-_pile_header_cover_description-wrap').siblings('.cmb_metabox_description').addClass('hero-visibility');

			if ( ! has_cover_description_content() && check_number_of_slides() < 1  ) {
				$('#pile_project_header_area_slideshow, #pile_page_header_area_cover').addClass('is--hidden').removeClass('is--visible');
			} else {
				$('#pile_project_header_area_slideshow, #pile_page_header_area_cover').addClass('is--visible').removeClass('is--hidden');
			}
		};
		...
		var toggleSlidesOptionsDisplay = function () {
			...
			$('#_pile_separator_slideshow_options, #_pile_page_slider_options_title, #_pile_portfolio_archive_slider_options_title').parents('.cmb-type.cmb-type-title').addClass('slideshow-area-title');

			if ( check_number_of_slides() > 1 ) {
				$('.slideshow-area-title').addClass('is--enabled').removeClass('is--disabled');

				$('#_pile_post_slider_autoplay, #_pile_post_slider_delay').each(function () {
		...
		$('#_pile_portfolio_featured_projects').on('change', function () {
			toggleSlidesOptionsDisplay();
		});
		...
		$('<span class="hero-hidden-overlay  dashicons  dashicons-hidden"></span>').insertAfter('#_pile_header_cover_description_ifr');
		...
		var $hero_color =  $('#_hero_background_color, #_pile_project_color');

		if ( $hero_color.length > 0 ) {
			pile_add_editor_bg_color( $hero_color.val() );

			$hero_color.on('wpcolorpicker:change', function ( ev ) {
				var color = $(this).val();
				pile_add_editor_bg_color(color);
			} );
		}
	});

	var pile_add_editor_bg_color = function( color ) {
		var $hero_desc_ifr = $('#wp-_pile_header_cover_description-wrap').find('iframe');
